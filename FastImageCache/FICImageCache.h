...
/**
 This method is called on the delegate whenever the image cache has a diagnostic message to log.
 
 @param imageCache The image cache that is requesting the source image.
 
 @param diagnosticMessage The diagnostic message generated by the image cache.
 
 @discussion Fast Image Cache will not explicitly log any messages to standard output. Instead, it allows the delegate to handle (or ignore) any diagnostic output.
 */
- (void)imageCache:(FICImageCache *)imageCache diagnosticMessage:(NSString *)diagnosticMessage;
